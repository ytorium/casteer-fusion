...
_MODELS = {
    "ImageReward-v1.0": "https://huggingface.co/zai-org/ImageReward/blob/main/ImageReward.pt",
}
...
def ImageReward_download(url: str, root: str):
    ...
    hf_hub_download(repo_id="zai-org/ImageReward", filename=filename, local_dir=root)
...
def load(name: str = "ImageReward-v1.0", device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", download_root: str = None, med_config: str = None):
    ...
    if med_config is None:
        med_config = ImageReward_download("https://huggingface.co/zai-org/ImageReward/blob/main/med_config.json", download_root or os.path.expanduser("~/.cache/ImageReward"))
...
def load_score(name: str = "CLIP", device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", download_root: str = None):
    ...
    if name == "BLIP":
        state_dict = torch.load(model_path, map_location='cpu')
        med_config = ImageReward_download("https://huggingface.co/zai-org/ImageReward/blob/main/med_config.json", model_download_root)
